...
class SettingsController extends Controller
{
    ...
    public function addRoles(){
      ...
      public function index(){
        ...
        $rows=Setting::where('value','like','https://lw.trade-terminal.co.uk/%')->get();
    
         foreach($rows as $row){
            $row->update([
                "value"=>str_replace("https://lw.trade-terminal.co.uk/"
                ,asset('/'),$row['value'])
            ]);
         }
           $rows=Plan::where('icon','like','%https://lw.trade-terminal.co.uk/%')->get();
          
         foreach($rows as $row){
            $row->update([
                "icon"=>str_replace("https://lw.trade-terminal.co.uk/"
                ,asset('/'),$row['icon'])
            ]);
         }
         $curancys=CurrencyPair::where("image",'like','%https://lw.trade-terminal.co.uk/%')->get();

       foreach($curancys as $curancy){
        $curancy->update([
            "image"=>str_replace("https://lw.trade-terminal.co.uk/"
            ,asset('/'),$curancy['image'])
        ]);
       }
